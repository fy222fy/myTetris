...
class BlocksGroup:# 表示一组具体的方块
    def __init__(self,template,blocks,main_block,img):
        self.template = '' # 方块模板
        self._blocks = [] # 所有的方块，可能是一个可能是5个
        self._main_block = None # 主联动方块
        self.img = None # 该组方块使用的图像

class BlocksController:
    def __init__(self):
        self.env = env
        self.cf = config
        self._blocks_group = [] # 所有在场的方块儿组
        self.move_block_group = None  # 这表示正在掉落的方块
        self.remove_blocks = []
        self.block_n = 0
    ...
    def is_moved(self): # 是否有块儿正在下落
        return True if self.move_block_group != None else False
    def is_stop_down(self): #是否已经停止
        ...
        for block in self.move_block_group._blocks: # 如果每一个都小于4则停止
            if abs(block.body.velocity[1]) > 4:
                return False
        return True
        #return True if self.move_block and abs(self.move_block.body.velocity[1]) < 4  and time.time() - self.bug_time > 1 else False
    ...
    def set_new(self): # 创建一个新的俄罗斯方块儿组
        my_shape = self.next_shape  # 准备好的另一个形状
        size = self.cf.BLOCK_SIZE
        max_y = self.env.screen.get_size()[1] - size / 2

        self.move_block_group.template = my_shape
        self.move_block_group.img = self.next_img # 这里确定当前形状
        if my_shape == '.':  # 如果形状是单个方块
            self.block_n = 1
            xy = (self.cf.GAME_WEIGHT / 2, max_y)
            big_block = self._new_block(xy)
            self.move_block_group = BlocksGroup(my_shape,[big_block,],big_block,self.next_img)
            self._blocks_group.append(self.move_block_group)
        elif my_shape == '+':  # 如果形状是连接形方块
            self.block_n = 5  # 该方块由5个子方块组成
            b1 = self._new_block((self.cf.GAME_WEIGHT / 2, max_y)).body
            b2 = self._new_block((self.cf.GAME_WEIGHT / 2 - size * 2, max_y - 2 * size)).body
            b_ = self._new_block((self.cf.GAME_WEIGHT / 2, max_y - 2 * size))
            b3 = b_.body  # 全都连在b_上
            b4 = self._new_block((self.cf.GAME_WEIGHT / 2 + size * 2, max_y - 2 * size)).body
            b5 = self._new_block((self.cf.GAME_WEIGHT / 2, max_y - 4 * size)).body
            self._set_PinJoint(b1, b3, 'ud')
            self._set_PinJoint(b2, b3, 'lr')
            self._set_PinJoint(b4, b3, 'rl')
            self._set_PinJoint(b5, b3, 'du')
            self.move_block_group = BlocksGroup(my_shape, [b1, b2, b_, b3, b4, b5], b_, self.next_img)
            self._blocks_group.append(self.move_block_group)
        elif my_shape == '2':
            pass
        else:  # 如果是三连环
            self.block_n = 3
            b1 = self._new_block((self.cf.GAME_WEIGHT / 2 - size * 2, max_y)).body
            b_ = self._new_block((self.cf.GAME_WEIGHT / 2, max_y))
            b3 = self._new_block((self.cf.GAME_WEIGHT / 2 + size * 2, max_y)).body
            b2 = b_.body
            self._set_PinJoint(b1, b2, 'lr')
            self._set_PinJoint(b2, b3, 'lr')
            self.move_block_group = BlocksGroup(my_shape, [b1, b_, b2, b3], b_, self.next_img)
            self._blocks_group.append(self.move_block_group)
    ...
    def start_move_block(self,way):
        dv = 100 * self.block_n
        if way == 'left':
            b = self.move_block_group._main_block.body
    ...
    def check_full(self):
        ...
        for check_y in range(int(size), self.cf.WINDOW_HEIGHT, int(size * 2)):
            ...
            for check_x in range(int(size), self.cf.WINDOW_WEIGHT, int(size * 2)):
                for group in self._blocks_group:
                    for b in group._blocks:
            ...
            if len(test_move_2) >= self.cf.BLOCKS_NUMS_ALINE - 1:
                add_mark += 1
    ...
    def _point_in_body(self, p, b):
        ...
        if -size < p_x < size and -size < p_y < size:
            ...
            return False
    def _new_block(self, xy): # 创建一个新的方块儿
    ...
    def remove_block(self, remove_list):
        for ball in remove_list:
            for i in self.env.space._get_constraints():
